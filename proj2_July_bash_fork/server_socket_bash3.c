...
exit(0) normal parent exit
exit(1) error parent exit
exit(2) normal child exit --> pID
exit(3) exit via exit_child_process --> pID
exit(4) child_child exit at exec_shell function --> pID2
exit(5) exit the cild process responsible for fgets(quit) --> pID3
*/
...
unsigned int cli_ID = 0; // As I need it in main and exit_child_process functions
char *user_pass = NULL; // As I need it in Authentication-Conversation and main functions ----> I think it's wrong position as may cause race-condition
char *tofree_result = NULL; // just pointer to point to the mem need to be freed
pid_t *pID_table = NULL; // As I need it to save all pID from fork to have all the child_client processes, used in main & term_handler & CHLD_handler

// Signal handler to terminate all the child processes
void term_handler (int signum)
{
    for (unsigned int i = 0; i < cli_ID; i++) 
    {
        fprintf(stdout, "pID_table[%d]: %d\n", i, pID_table[i]);
        kill(pID_table[i], SIGTERM);
    }
    int status;
    pid_t wait_re;
    // wait for child process to reap it and not creating zombie processes
    while (wait_re = waitpid(-1, &status, 0) > 0)
    {
        if (WIFEXITED(status)) {
            fprintf(stdout, "Child pID %d Terminated Normally with status %d\n", wait_re, status);
        }
        else {
            fprintf(stdout, "Child pID %d Terminated Abnormally with status %d\n", wait_re, status);
        }
    }
...
// Signal handler to reap the child processes
void CHLD_handler (int signum)
{
    int status;
    pid_t wait_re;
    // wait for child process to reap it and not creating zombie processes
    while (wait_re = waitpid(-1, &status, WNOHANG) > 0)
    {
        for (unsigned int i = 0; i < cli_ID; i++)
        {
            if(wait_re == pID_table[i])
            {
                if (WIFEXITED(status)) {
                    fprintf(stdout, "Child pID %d Terminated Normally with status %d\n", wait_re, status);
                }
...
void error (const char *msg, int fd)
{
    ...
    perror(msg);
    printf("\n");
    // If the parent is about to close, kill all children
    for (unsigned int i = 0; i < cli_ID; i++) 
    {
        fprintf(stdout, "pID_table[%d]: %d\n", i, pID_table[i]);
        kill(pID_table[i], SIGTERM);
    ...
    {
        if (WIFEXITED(status)) {
            printf("Child pID %d Terminated Normally with status %d\n", wait_re, status);
        }
        else {
            printf("Child pID %d Terminated Abnormally with status %d\n", wait_re, status);
        }
    }
    if (pID_table != NULL) free(pID_table);
    exit(1);
...
{
    if ( send(fd, str, strlen(str)-1, 0) < 0 )
    {
...
int main (int argc, char *argv[])
{
    ...
    // Make sigaction to handle the sigterm will be sent if user quit
    ...
    sa_term.sa_handler = CHLD_handler;
    ...
    // If the parent recieved SIGCHLD, reap the child process printing its pID & status
    ...
    // Fork to make a process for fgets QUIT
    pid_t pID3 = fork();
    if (pID3 < 0)
    {
    ...
    {   
        ...
        if(connect_client_SocketFD < 0 && errno != EINTR)
        {
            perror("[-]Error in Accepting.");
            printf("\n");
            continue;
        }
        else if (connect_client_SocketFD < 0 && errno == EINTR)
        {
            error("Sys Call Interrupted,", server_socketFD);
        }
        else
        {
        ...
        {    
            ...
            // If the child recieved SIGTERM, it should handle it by exiting and free resources
            // No need as SIGTERM exit the program cleanly & when the client recieve empty msg, it will also exit with notification
            // I mean we handle it at the client side
            // signal(SIGTERM, exit_child_process("Parent termination,", connect_client_SocketFD, cd_home, user) );
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    if (rec < 0) exit_child_process ("Receiving Failed,", connect_client_SocketFD, cd_home, user);
                    else
                    {
                        // Making sure the newline is replaced by NULL - from man fgets, newline is being stored
                        if (strlen(buffer) > 0)
                        ...
                        else 
                        {
                            ...
                            // To solve if there's any remaining data in the socket unsent
        ...
        {
            // Adding pID to the table
            pID_table = (int *)realloc(pID_table, cli_ID*sizeof(int));
            pID_table[cli_ID - 1] = pID;
            // for (int i = 0; i < cli_ID; i++) printf("pID_table[%d]: %d\n", i, pID_table[i]);
...
char *exec_shell(const char *cmd) {
    ...
    {  
        ...
        // Change the file descriptor flag to EXIT_ON_EXEC as after exec the fd[1] will still be opened
        int flags = fcntl(fd[1], F_GETFD);
        if (flags < 0) {
            perror("Error with F_GETFD.");
            exit(4);
        }
        if ( fcntl(fd[1], F_SETFD, flags | FD_CLOEXEC) < 0 ) {
            perror("Error with F_SETFD.");
            exit(4);
        }
