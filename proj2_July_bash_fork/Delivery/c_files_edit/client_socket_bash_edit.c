...
int main (int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        ssize_t r_read = -1;
        char *r_command = calloc(sizeof(char), 1024);
        int counter = 2;
        while (true)
        {
            r_read = read(client_socketFD, buffer, 1024);
            // printf("r_read: %d\n", r_read);
            // printf("first_read = %B\n", first_read);
            if ( r_read > 0)
            {
                strcat(r_command, buffer);
                if (r_read >= 1024)
                {
                    // We made a tmp pointer to be able to free r_command if failure happened. I think realloc do that for us, no need
                    char *tmp = (char *)realloc(r_command, counter*1024);
                    if (tmp == NULL)
                    {
                        free(r_command);
                        r_command = "Error with realloc. Your Command wasn't executed!\n"; 
                    }
                    r_command = tmp;
                } 
                else 
        ...
        /*As we programmed the server to send always result, evenif the command was has no output like mkdir 
        Then, if buffer was empt, it means the child process from the server has terminated by quit or by error */
        if (strlen(r_command) == 0)
        {
            error("[-]Socket disconnected, try to connect again!", client_socketFD);
        }
        
        printf("Msg received from server:\n %s\n", r_command);
        free(r_command);

        // To delete the remaining data if any result wasn't printed for any reason
        fflush(fdopen(client_socketFD, "w"));
        fflush(stdout);
    }
