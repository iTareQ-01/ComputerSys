...
char *user_pass = NULL; // As I need it in Authentication-Conversation and main functions
char *tofree_result = NULL; // just pointer to point to the mem need to be freed
// int *pID_table = NULL; // As I need it to save all pID from fork to have all the child_client processes, used in main & term_handler & CHLD_handler
...
void term_handler (int signum)
{
    ...
    {
        pid_t wait_re = waitpid(-1, &status, 0);
        if (wait_re < 0) printf("Error waiting for a child\n");
        else {
            if (WIFEXITED(status)) {
...
void CHLD_handler (int signum)
{
    ...
    for (int i = 0; i < cli_ID; i++) 
    {
        ...
        if (wait_re < 0) printf("Error waiting for a child\n");
            ...
            if (WIFEXITED(status)) printf("Child pID %d Terminated Normally with status %d\n", wait_re, status);
            else printf("Child pID %d Terminated Abnormally with status %d\n", wait_re, status);
        }
    }
    return;
}
...
int main (int argc, char *argv[])
{
    ...
    for (int i = 0; i < cli_ID; i++) 
    {
        pid_t wait_re = waitpid(-1, &status, WNOHANG);
        if (wait_re < 0) printf("Error waiting for a child\n");
        else {
            if (WIFEXITED(status)) printf("Child pID %d Terminated Normally with status %d\n", wait_re, status);
            else printf("Child pID %d Terminated Abnormally with status %d\n", wait_re, status);
