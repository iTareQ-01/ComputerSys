...
int main (int argc, char *argv[])
{
    ...
    if (pID3 == 0)
    {
        // fcntl(0, F_SETFL, fcntl(0, F_GETFL) | O_NONBLOCK); ----> It's a separate process so, no need to unblock
        // sleep(4);
...
// function opens and writes to a file to pipe contents of an exec call to /bin/sh/
//     FILE* pipe = popen(cmd, "r");   ---> Just to remember, we didn't use it
// char* popen_shell(const char* cmd) {
//     char sh_buffer[1024];
//     char* sh_result = malloc(1024);
//     if (sh_result == NULL) {
//         return NULL;
//     }
//     printf("cmd: %s", cmd);
...
//     }
//     while (!feof(pipe)) {
//         if (fgets(sh_buffer, 1024, pipe) != NULL) {
...
char *exec_shell(const char *cmd) {
    ...
    else
    {
        ...
        while (true)
        {
            ...
            {
                strcat(sh_result, sh_buffer);
                // I tried this first, but read return 332 char and the last one is not /0 or EOF.. it was /n 
                // if (sh_result[strlen(sh_result)] != '\0' || sh_result[strlen(sh_result)] != EOF)
                if (r_read >= 1024)
                {
...
char *authenticate(const char* username)
{
    ...
    */
    ...
    // buf = malloc(bufsize);
    // if (buf == NULL) {
    //     exit_child_process ("Error with malloc during getting user info", fd);
    // }

    // s = getpwnam_r(username, &pwd, buf, bufsize, &result);
    // if (result == NULL) {
    //     if (s == 0)
    //         exit_child_process ("Username isn't found", fd);
    //     else {
    //         errno = s;
    //         perror("getpwnam_r");
    //         exit_child_process ("Error getting user info", fd);
    //     }
    // }
    // free(buf);
    // printf("Name: %s; Passwd: %s\n", pwd.pw_gecos, pwd.pw_passwd);
}